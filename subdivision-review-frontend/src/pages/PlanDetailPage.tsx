...
export function PlanDetailPage() {
    ...
    useEffect(() => {
        ...
        getPlan(planId).then(setPlan).catch((err) => setLoadError(apiErrorMessage(err, "Could not load this plan")));
    }, [planId]);
    async function handleReview() {
        ...
        try {
            ...
            getPlan(planId).then(setPlan).catch(() => {});// Re-fetch since the backend still flips status to 'failed' server-side
        } finally {
            setReviewing(false);
        }
    }
    async function handleDelete() {
        if (!planId || !window.confirm("Delete this plan? This cannot be undone.")) {
            return;
        }
    ...
    if (loadError) {
        return <p className="page form-error">{loadError}</p>;
    }
    if (!plan) {
        return <p className="page page-loading">Loading plan…</p>;
    }
    ...
    return (
        <div className="page">
            <div className="page-header">
                ...
                <button type="button" className="btn-ghost btn-danger" onClick={handleDelete}>Delete plan</button>
            </div>
            {plan.review_status !== "completed" && (
                <div className="card-form review-trigger">
                    <label className="field">
                        <span>Jurisdiction (optional)</span>
                        <input value={jurisdiction} onChange={(e) => setJurisdiction(e.target.value)} placeholder="e.g. City of Springfield — Title 18"/>
                    </label>
                    {reviewError && <p className="form-error" role="alert">{reviewError}</p>}
                    <button type="button" className="btn-primary" onClick={handleReview} disabled={reviewing}>{reviewing ? "Reviewing… this can take up to 30s" : "Run AI review"}</button>
                </div>
            )}
            {notes && (
